...
function init_controllers_common()
{
    ...
    while (($inc_file = readdir($includes_dir)) != false) {
        if (strstr($inc_file, '.php'))
            require('./includes/controllers_common/' . $inc_file);
    }
}
...
function init_controllers_call()
{
    ...
    while (($inc_file = readdir($includes_dir)) != false) {
        if (strstr($inc_file, '.php'))
            require('./includes/controllers_call/' . $inc_file);
    }
}
...
function phone_formatting($phone)
{
    if (preg_match('~^[78]\d{10}$~', $phone))
        $phone = preg_replace('~^([78])(\d{3})(\d{3})(\d{2})(\d{2})$~', '+$1 ($2) $3-$4-$5', $phone);
    return $phone;
}
